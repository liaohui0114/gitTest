...
public class CoopCampaignMenu : BaseMenu
{
    [SerializeField]
    ...
    public void UpdateCampaigns()
    {
        ...
        for (int i = 0; i < App.CoopCampaigns.Count; i++)
        {
            CampaignIcon item = (CampaignIcon) UnityEngine.Object.Instantiate(this._campaignIconPrefab);
            item.transform.parent = this._parent;
            item.CampaignIndex = App.CoopCampaigns.Keys[i];
            item.CanBeBought = item.CampaignIndex == (PlayerSettings.OpenedCoopCampaign.Value + 1);
            item.IsEnabled = item.CanBeBought || (item.CampaignIndex <= PlayerSettings.OpenedCoopCampaign.Value);
            item.transform.localPosition = new Vector3((float) (item.CampaignIndex * 40), 0f, 0f);
            this._campaigns.Add(item);
            if (!this._inited && (item.CampaignIndex == PlayerSettings.LastCoopCampaign.Value))
            {
